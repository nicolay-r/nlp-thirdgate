...
class Qwen2(BaseLM):
    ...
    def __init__(self, model_name, temp=0.1, device='auto',
    ...
    def ask(self, prompt):
        ...
        inputs = self.__tokenizer([text], return_tensors="pt").to(self.__device)
        outputs = self.__model.generate(**inputs, max_length=self.__max_length, temperature=self.__temp, do_sample=True)
        generated_ids = [
